...
const KPICardPro = ({ metric, loading = false }: { metric: KPIMetric; loading?: boolean }) => {
  ...
  return (
    <motion.div
      ...
      className="h-[180px]"
    >
      <Card className="bg-white border-gray-200 shadow-sm hover:shadow-md transition-all h-full">
        <CardContent className="p-6 h-full flex flex-col justify-between">
          <div>
            <div className="flex items-start justify-between mb-2">
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider truncate max-w-full pr-2">
                {metric.title}
              </h3>
              {metric.trend && (
    ...
    <Card className="bg-white border-gray-200 shadow-sm">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-lg font-semibold text-gray-900">Pipeline Flow</CardTitle>
            <div className="text-xs text-gray-500">From daily activity metrics</div>
          </div>
          ...
          <FunnelChart margin={{ top: 10, right: 20, bottom: 10, left: 20 }}>
            <defs>
              {pipelineData.map((_, index) => (
                <linearGradient key={index} id={`pipeGradient${index}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={pipelineData[index].color} stopOpacity={0.95} />
                  <stop offset="100%" stopColor={pipelineData[index].color} stopOpacity={0.75} />
                </linearGradient>
              ))}
            </defs>
            <Tooltip 
        ...
        <div className="flex justify-between items-center px-8 mt-3">
          {pipelineData.slice(1).map((stage, index) => (
            ...
            >
              ...
              <span className="text-xs font-medium" style={{ color: stage.conversionRate > 50 ? COLORS.success : stage.conversionRate > 30 ? COLORS.warning : COLORS.danger }}>
                {stage.conversionRate}%
              </span>
            </motion.div>
          ))}
...
const TeamPerformanceMatrix = ({ data }: { data: any }) => {
  const teamData = useMemo(() => {
    if (!data?.teamPerformance || data.teamPerformance.length === 0) {
      return [
        { name: 'No Data', conversion: 0, activity: 0, roi: 0, x: 0, y: 0 }
      ];
    }
    ...
    const userPerformance = data.teamPerformance.reduce((acc: any, metric: any) => {
      ...
      if (!acc[userId]) {
        acc[userId] = {
          name: `User ${userId.slice(0, 8)}`, // Use partial ID as name
          conversions: [],
          activities: [],
          scores: []
        };
  ...
  const items = useMemo(() => {
    ...
    insights.teamPerformance.forEach((performance: any) => {
      ...
      if (conversionRate > 30) {
        coachingItems.push({
          type: 'success',
          title: `High conversion rate: ${conversionRate.toFixed(1)}%`,
      ...
      if (activityVolume < 50) {
        coachingItems.push({
          type: 'warning',
          title: `Low activity volume: ${activityVolume} activities`,
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [
          ...
          // Team performance with date filter
          supabase
  ...
  const kpiMetrics: KPIMetric[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        trend: avgROI > 300 ? 'up' : avgROI > 200 ? 'neutral' : 'down',
        trendValue: `Avg from ${aiImpact.length} records`,
        comparison: 'Target: 300-500%'
      },
      {
        title: 'Cost Per Qualified Lead',
    ...
    <div className="h-screen overflow-y-auto bg-gray-50">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <TeamPerformanceMatrix data={dashboardData} />
        </div>
